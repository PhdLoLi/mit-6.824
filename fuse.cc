...
getattr(yfs_client::inum inum, struct stat &st)
{
  ...
  if(yfs->isfile(inum)){
     ...
     printf("getattr -> size:%llu\tatime:%lu\tmtime:%lu\tctime:%lu\n", info.size,info.atime,info.mtime,info.ctime);
   } else {
                   ...
                   int to_set, struct fuse_file_info *fi)
{
  ...
  if (FUSE_SET_ATTR_SIZE & to_set) {
    ...
    // You fill this in for Lab 2
		...
		if(yfs->isdir(inum) || attr->st_size<0)
		{
		...
		if(attr->st_size ==0 )
			ret = yfs->truncate_zero(inum);
		else
			ret = yfs->truncate_size(inum, attr->st_size);

		if(ret != yfs_client::OK || getattr(inum, st)!= yfs_client::OK)
			fuse_reply_err(req, ENOSYS);
		else
			fuse_reply_attr(req, &st, 0);
                 ...
                 struct fuse_file_info *fi)
{
  ...
  // Change the above line to "#if 1", and your code goes here
	yfs_client::inum inum =  ino;
	yfs_client::status ret;
	std::cout<<"inum: "<<inum<<"buf: "<<buf<<"size: "<<size<<"off: "<<off<<std::endl;
	ret = yfs->write(inum, size, off, std::string(buf,size));
	if(ret == yfs_client::OK)
                        ...
                        mode_t mode, struct fuse_entry_param *e)
{
  ...
  e->generation = 0;
	...
	// You fill this in for Lab 2
	yfs_client::status ret;
	yfs_client::inum inum_parent = parent;
	yfs_client::inum inum;
	std::string buf;
	std::cout<<"fuseserver_createhelper Here! inum_parent: "<<inum_parent<<"\tname: "<<name<<std::endl;

	ret = yfs->lookup(inum_parent, name, inum,1,buf);
	if (ret == yfs_client::OK)
		return yfs_client::EXIST;
	else if(ret == yfs_client::IOERR)
		return ret;
	ret = yfs->createfile(inum_parent, name, inum, buf);
	e->ino = inum;
	printf("fuseserver_createhelper result e->ino:%016lx\n",e->ino);
	if(ret == yfs_client::OK)
	{
		ret = getattr(e->ino, e->attr);
		std::cout<<"e-attr(a,m,c) "<<(e->attr).st_atime<<" "<<(e->attr).st_mtime<<" "<<(e->attr).st_ctime<<std::endl;
	}
...
fuseserver_lookup(fuse_req_t req, fuse_ino_t parent, const char *name)
{
  ...
  // You fill this in for Lab 2
	...
	std::cout<<"fuseserver_lookup Here! inum_parent: "<<inum_parent<<"\tname: "<<name<<std::endl;
	ret = yfs->lookup(inum_parent, name, inum, 0, buf);
                   ...
                   off_t off, struct fuse_file_info *fi)
{
  ...
  // You fill this in for Lab 2
	yfs_client::status ret;
	std::vector<yfs_client::dirent> dirent_vec;
	std::vector<yfs_client::dirent>::iterator it;
	fuse_ino_t temp;
	
	ret = yfs->readdir(inum, dirent_vec);
	if(ret != yfs_client::OK)
		goto release;
	for(it = dirent_vec.begin(); it != dirent_vec.end(); it++)
	{
		temp = it->inum;
		printf("inum : %016lx\t",temp);
		std::cout<<it->name<<std::endl;
		dirbuf_add(&b, it->name.c_str(), temp);
	}
     ...
     mode_t mode)
{
  ...
  // You fill this in for Lab 3
	...
	ret = yfs->lookup(inum_parent, name, inum, 2, buf);
	if (ret == yfs_client::OK)
	{
		fuse_reply_err(req, EEXIST);
		return;
	...
	ret = yfs->mkdir(inum_parent, name, inum, buf);
	e.ino = inum;
	printf("fuseserver_mkdir result e->ino:%016lx\n",e.ino);
	if(ret == yfs_client::OK)
	{
		ret = getattr(e.ino, e.attr);
		std::cout<<"e-attr(a,m,c) "<<(e.attr).st_atime<<" "<<(e.attr).st_mtime<<" "<<(e.attr).st_ctime<<std::endl;
...
fuseserver_unlink(fuse_req_t req, fuse_ino_t parent, const char *name)
{
  ...
  // Not found:	fuse_reply_err(req, ENOENT);
	...
	ret = yfs->lookup(inum_parent, name, inum, 3, buf);
	...
	if(ret == yfs_client::OK)
	{
		if((inum & 0x80000000) == 0)
		{
			fuse_reply_err(req, ENOSYS);
			return;
		}
		ret = yfs->unlink(parent, inum, name, buf);
		if(ret == yfs_client::OK)
		{
