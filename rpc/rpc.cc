...
//			std::cout<<"case DONE!"<<sz1<<std::endl;
			c->send(b1, sz1);
			break;
		...
		unsigned int xid_rep, char **b, int *sz)
{
	...
	{
		reply_t one(xid);
		reply_window_[clt_nonce].push_back(one);
		return NEW;
	}

	if(xid_rep > max_xid_rep[clt_nonce])
		max_xid_rep[clt_nonce] = xid_rep;

	for(it = clt->second.begin(); it != clt->second.end(); )
	{
		if((*it).xid <= max_xid_rep[clt_nonce] && (*it).buf && (*it).cb_present)
		{
			free((*it).buf);
	...
	for(it = clt->second.begin(); it != clt->second.end(); it++)
	{
		if ((*it).xid > xid)
		{
			reply_window_[clt_nonce].insert(it,one);
			break;
		}
	}
...
void
rpcs::test(unsigned int clt_nonce, unsigned int xid)
{
	std::list<reply_t>::iterator it;
